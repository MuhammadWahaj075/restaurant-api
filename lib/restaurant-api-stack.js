...
class RestaurantApiStack extends Stack {
  constructor(scope, id, props) {
    ...
    const restaurants = api.root.addResource('restaurants'); // /restaurants
    restaurants.addMethod('POST', new apigateway.LambdaIntegration(createFn)); // POST /restaurants
    restaurants.addMethod('GET', new apigateway.LambdaIntegration(listFn)); // GET /restaurants

    const restaurant = restaurants.addResource('{id}'); // /restaurants/{id}
    restaurant.addMethod('GET', new apigateway.LambdaIntegration(getFn)); // GET /restaurants/{id}
    restaurant.addMethod('PUT', new apigateway.LambdaIntegration(updateFn)); // PUT /restaurants/{id}
    restaurant.addMethod('DELETE', new apigateway.LambdaIntegration(deleteFn)); // DELETE /restaurants/{id}
